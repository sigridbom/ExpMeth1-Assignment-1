...
I will subset the data into the two different conditions because the data set represents two different groups.  
...
### histogram condition 1 (semantically contextual word)
con1_hist <- ggplot(con1, aes(x = Reaction_time)) 
con1_hist + geom_histogram(aes(y = ..density..), binwidth = 0.25) + stat_function(fun = dnorm, args = list(mean = mean(con1$Reaction_time), sd = sd(con1$Reaction_time)), colour = "blue", size = 1) +
...
### histogram condition 2 (salient word)
con2_hist <- ggplot(con2, aes(x = Reaction_time)) 
con2_hist + geom_histogram(aes(y = ..density..), binwidth = 0.25) + stat_function(fun = dnorm, args = list(mean = mean(con2$Reaction_time), sd = sd(con2$Reaction_time)), colour = "red", size = 1) +
...
The histograms and the QQ-plots look fairly okay, but not necessesarily normally distributed. The values of skew.2SE and kurt.2SE for condition 1 are too high for a normal distribution (in this case 7,12 and 1,81 respectively). The same applies for condition 2 but with the values 9,58 (skew.2SE) and 11,19 (kurt.2SE). 
Because both distributions have a positive value of kurt.2SE they are leptokurtic, which is not normal.

The value Shapiro Wilk's test for condition 1 is 0,95 with a significant p-value (p<0,05), which means that the data is significantly different from a normal distribution. The same applies for condition 2 except the value of the test is 0,94 (p<0,05). 
...
con1_out_log <- con1_out %>% mutate(log(con1_out$Reaction_time))
con2_out_log <- con2_out %>% mutate(log(con2_out$Reaction_time))


### new histogram condition 1
con1_hist_log <- ggplot(con1_out_log, aes(x = Reaction_time)) 
con1_hist_log + geom_histogram(aes(y = ..density..), binwidth = 0.25) + stat_function(fun = dnorm, args = list(mean = mean(con1_out_log$Reaction_time), sd = sd(con1_out_log$Reaction_time)), colour = "blue", size = 1) +
  theme_minimal() + labs(x = "Reaction time", y = "Density") + ggtitle("Condition 1 reading time distribution transformed")

## new qq-plot for condition 1
qq_plot_con1_log <- ggplot(con1_out_log, aes(sample = Reaction_time)) 
qq_plot_con1_log + stat_qq() + stat_qq_line(colour = "blue") + ggtitle("Condition 1 reading time qq-plot transformed")
  

### new histogram condition 2 
con2_hist_log <- ggplot(con2_out_log, aes(x = Reaction_time)) 
con2_hist_log + geom_histogram(aes(y = ..density..), binwidth = 0.25) + stat_function(fun = dnorm, args = list(mean = mean(con2_out_log$Reaction_time), sd = sd(con2_out_log$Reaction_time)), colour = "red", size = 1) +
  theme_minimal() + labs(x = "Reaction time", y = "Density") + ggtitle("Condition 2 reading time distribution transformed")

## new qq-plot for condition 2
qq_plot_con2_log <- ggplot(con2_out_log, aes(sample = Reaction_time)) 
qq_plot_con2_log + stat_qq() + stat_qq_line(colour = "red") + ggtitle("Condition 2 reading time qq-plot transformed")
  
#checking numeric values for both conditions again
round(pastecs::stat.desc(cbind(con1_out_log$Reaction_time, con2_out_log$Reaction_time), basic = F, norm = TRUE), digits = 2)

```

The histograms and the qq-plots are looking a lot better, especially the condition 2 histogram. 
The values of skew.2SE and kurt.2SE are the same for both conditions as before the transformation. The same goes for the Shapiro Wilk's test.
...
```{r word length and correlation - devide the data into subset with the different conditions?????}
...
scat_len + geom_point() + geom_smooth(method = lm, colour = "red") + ggtitle("Correlation between word length and reaction time")
...
Even though the results are significant the effect size is so small that the correlation is not very strong at all. 
...
scat_freq + geom_point() + geom_smooth(method = lm, colour = "red") + ggtitle("Correlation between word frequency and reaction time")
...
scat_num + geom_point() + geom_smooth(method = lm, colour = "red") + ggtitle("Correlation between word number and reaction time")
...
This makes sense because I observed many participants going slowly in the beginning and then clicking faster and faster to get to the next word. 

In conclusion all three word properties are significant, but is only the word number i.e. the order of words in the story that has a small effect size, where the two others are too small to actually conclude there is a meaningful correlation. 
...
First I will check the assumptions again because we now have four groups of data; the two versions and within the two version the semantically contextual word or the salient word (i.e. word number 106) and the word following that (word number 107).
...
The values of skew.2SE for all four data sets are between 0-1 and the values of kurt.2SE are all between -1 and 1 which is supporting a normal distribution. 
...
I will therefore continue with a parametric t.test. 
...
```
## Comparing the reaction of the two conditions according to the expected and the unexpected word

When conducting an independent t-test I have found that the average reading time for expected and unexpected word is not significantly increased by the unexpected word, t(23.98) = -0.63, p > .05, (mean expected = 0.57, mean unexpected = 0.50)

The same goes for the word following the expected and unexpected word, t(22.62) = 1.01, p > .05, (mean expected = 0.48, mean unexpected = 0.56).
...
- should I continue with the subsets of the two conditions throughout the entire part 1? (word length, frequency and number)